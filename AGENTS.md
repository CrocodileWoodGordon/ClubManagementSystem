...
- 2026-02-13：完善考勤导入重复记录合并规则，导入同一学生同一课次多条记录时按最严重状态落库并新增重复场景单测。
- 2026-02-23：更新 `README.md`，区分内部开发与客户离线部署流程，新增“客户离线部署说明”章节，指导通过 `docker load` 导入前端/后端/数据库镜像后使用 `docker compose up -d --no-build` 启动，确保客户全程无需外网。
