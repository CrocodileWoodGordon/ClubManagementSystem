...
1. **基础数据准备**: 通过“学生名单 Excel 导入”接口上传“校区/班级/姓名”三列，系统会为缺失的 `homerooms` 建档并写入 `students`，学年默认使用当前激活学期的 `start_date` 年份。学生/班级数据与 `term_id` 强绑定，切换学期后需重新导入或复用旧学期数据。
- **学生名册维护**: 管理员在“学生名册”页面选择学期 + 校区后，可查看班级列表、编辑班主任/备注等信息，并对单个学生执行新增、修改、删除操作。若当前学期尚无数据，可指定来源学期一键复用班级与学生名单。
...
* **占位文本**: 通过 `/api/import/placeholders` 可查询或更新 Excel 中代表“空报名”的占位字符串（默认包含 `-`、`(空)`、`(跳过)` 等），避免导入时生成无效社团。
* **处理逻辑**:
...
* **前端交互**：在“班级分配”页面中，管理员点击“开始分班”后即可看到该组合下的报名名单与班级列表，可随时新建班级（配置时间、地点、容量），每个学生行尾提供下拉选择班级并支持“多选模式”批量应用。
